...
mod model_repository {
  ...
  pub async fn should_find_owners_repositories() {
    ...
    let repositories = database::repository::find_repositories_by_owner_id(&user._id, pagination_argument)
      .await
      .unwrap();
  ...
  pub async fn should_convert_a_repository_list_into_cursor_connection_of_repositories() {
    ...
    let organization = database::organization::find_organization_by_login(&owner_login)
      .await
      .unwrap()
      .unwrap();
    ...
    let repositories = database::repository::find_repositories_by_owner_id(&organization._id, pagination_arguments)
      .await
      .unwrap();

    let cursor_connection =
      database::repository::repositories_to_cursor_connection(&organization._id, Ok(repositories))
        .await
        .unwrap();
  ...
  pub async fn should_paginating_repositories_from_start_to_end() {
    ...
    mock::setup(&sufix).await;
    let login = format!("organization_acme_{sufix}");
    let owner = database::organization::find_organization_by_login(&login).await;
    ...
    let repositories = database::repository::find_repositories_by_owner_id(&owner._id, pagination_arguments)
      .await
      .unwrap();
