...
async fn should_convert_a_user_list_into_cursor_connection_of_users() {
  ...
  let users = database::organization::find_people_by_login(&db, &organization_login, pagination_argument)
    .await
    .unwrap();

  let cursor_connection =
    database::organization::organizations_users_to_cursor_connection(&db, &organization_login, Ok(users))
      .await
      .unwrap();
