use crate::infrastructure::{
  ...
  http_server::{route::organization, utils::HttpError},
  mock,
};
...
#[actix_rt::test]
async fn should_not_find_people_of_the_organization_when_the_org_is_empty() {
  let sufix = mock::random_sufix();
...
#[actix_rt::test]
async fn should_not_find_repositories_of_the_organization_when_the_org_does_not_have_one() {
  let sufix = mock::random_sufix();
  let login = format!("empty_org_{sufix}");
  let res = mock::make_request(
    mock::HttpMethod::Get,
