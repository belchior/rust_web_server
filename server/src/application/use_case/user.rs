...
pub async fn find_repositories(
  ...
  match result {
    ...
    Ok(None) => Ok(None),
    Ok(Some(owner)) => {
      let result = database::repository::find_repositories_by_owner_id(&owner._id, pagination_arguments).await;
      let cursor = database::repository::repositories_to_cursor_connection(&owner._id, result).await;
      match cursor {
        Err(err) => Err(AppError::Database(err)),
        Ok(cursor) => Ok(Some(cursor)),
      }
